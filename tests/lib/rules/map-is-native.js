...
const invalidCode =
  'const testArray = new Array(3);const result = _.map(testArray, (props) => {return props});const lastArr = [];';

const varObj = 'const testObj = {a: 1};_.map(testObj, (prop) => prop)';
  ...
  invalid: [
    {
      ...
      output:
        'const variable = Array.isArray(collection) ? collection.map(fn) : _.map(collection, fn)',
      errors: [
        {
